...
def model_3_a_i(niter,  par_prior, Yobs, W, nburn=None, thin=1, seed=None, theta_start=None, save_results=False):
    '''
    ...
    if theta_start is None:
        theta = {'mu_c' : np.mean(Yobs[W == 0]) + rnorm(0, 0.1, 1), # Y_c is normal with avg = mu_c
                 'mu_t' :  np.mean(Yobs[W == 1]) + rnorm(0, 0.1, 1), # Y_t is normal with avg = mu_t
                 'sigma2_c' : np.var(Yobs[W == 0]) + rnorm(0, 0.1, 1), # Y_c is normal with var = sigma2_c
                 'sigma2_t' : np.var(Yobs[W == 1]) + rnorm(0, 0.1, 1) # Y_t is normal with var = sigma2_t
                 }
    else:
        theta = theta_start
    ...
    for ell in range(1,niter+1):
        ...
        theta['mu_c'] = rnorm(nu_c_obs, np.sqrt(omega2_c_obs), 1)
        ...
        theta['mu_t'] = rnorm(nu_t_obs, np.sqrt(omega2_t_obs), 1)
        ...
        theta['sigma2_c'] =  (a_c_obs * b2_c_obs) / rchisq(a_c_obs, 1)
        ...
        theta['sigma2_t'] =  (a_t_obs * b2_t_obs) / rchisq(a_t_obs, 1)
